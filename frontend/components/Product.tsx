...
const Product = ({ product, className, ...props }: Props) => {
  return (
    <div className={`hover:shadow-2xl shadow-black product-card inline-block transition-[250ms] sm:w-[49%] justify-center overflow-x-hidden p-2 ${className}`}>
      <Link href={`/shop/product/${product._id}`} className='text-black'>
        <div className="">
          ...
          />
          <h4 className='mt-2 font-normal text-xl truncate'>{product.title}</h4>
        </div>
