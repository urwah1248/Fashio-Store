  ...
  return (
    <div className='py-10 w-full flex flex-col items-center'>
        ...
        <div className='scrollbar-hide overflow-x-scroll overflow-y-hidden w-full md:overflow-x-hidden md:flex flex-wrap md:gap-3 whitespace-nowrap justify-center'>
          ...
          .map((product,index)  => {
              return (
                  <Product className="mr-6 w-5/6 max-w-[268px] md:w-[300px] md:mr-0" key={index} product={product}/>
              )
          })}
        </div>
        {
        ...
        <Link href={`/shop/${category}`}>
          <Button className='mt-4 border border-black bg-zinc-900 text-gray-200 hover:text-zinc-900 hover:bg-gray-200' size="lg">View More</Button>
        </Link>
        }
    </div>
  )
